;; what is refs and how it works?

;; https://clojure.org/reference/refs
;; While Vars ensure safe use of mutable storage locations via thread isolation,
;; transactional references (Refs) ensure safe shared use of mutable storage locations
;; via a software transactional memory (STM) system.
...
;; Clojure transactions should be easy to understand if you’ve ever used database transactions -
;; they ensure that all actions on Refs are atomic, consistent, and isolated.
;; Atomic means that every change to Refs made within a transaction occurs or none do.
;; Consistent means that each new value can be checked with a validator function
...
;; Another feature common to STMs is that, should a transaction
